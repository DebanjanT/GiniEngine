    ...
    if (m_Scene && m_SelectedEntity != NullEntity) {
        ...
        if (ImGui::BeginPopup("AddComponent")) {
            if (ImGui::MenuItem("Sprite")) {
                if (!m_Scene->GetWorld().HasComponent<SpriteComponent>(m_SelectedEntity)) {
...
void PropertiesPanel::DrawComponents(Entity entity) {
    ...
    if (world.HasComponent<SpriteComponent>(entity)) {
        ...
        bool open = ImGui::TreeNodeEx((void*)typeid(SpriteComponent).hash_code(), flags, "Sprite");
...
void PropertiesPanel::DrawVec3Control(const std::string& label, Vec3& values, f32 resetValue, f32 columnWidth) {
