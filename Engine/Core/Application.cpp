...
Application::Application(const EngineConfig &config) : m_Config(config) {
  ...
  if (RendererAdapter::GetInstance().InitializeDiligentEngineWithWindow(
          m_Window->GetNativeWindow(), windowProps.width, windowProps.height)) {
    GINI_INFO(
        "Diligent Engine successfully initialized with window integration");
  } else {
    GINI_WARN("Diligent Engine initialization failed, using OpenGL fallback");
  }
