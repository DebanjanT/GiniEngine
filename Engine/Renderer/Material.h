...
public:
  ...
  static Ref<Material> LoadFromDirectory(const std::filesystem::path& directory);
  static Ref<Material> Load(const std::filesystem::path& materialFile);
  void Save(const std::filesystem::path& path);
  
  // Texture setters
  void SetAlbedoTexture(Ref<Texture2D> texture) { m_AlbedoTexture = texture; m_HasAlbedoTexture = texture != nullptr; }
  void SetNormalTexture(Ref<Texture2D> texture) { m_NormalTexture = texture; m_HasNormalTexture = texture != nullptr; }
  void SetRoughnessTexture(Ref<Texture2D> texture) { m_RoughnessTexture = texture; m_HasRoughnessTexture = texture != nullptr; }
  void SetMetallicTexture(Ref<Texture2D> texture) { m_MetallicTexture = texture; m_HasMetallicTexture = texture != nullptr; }
  void SetAOTexture(Ref<Texture2D> texture) { m_AOTexture = texture; m_HasAOTexture = texture != nullptr; }
  void SetHeightTexture(Ref<Texture2D> texture) { m_HeightTexture = texture; m_HasHeightTexture = texture != nullptr; }
...
public:
  ...
  Ref<Material> Load(const std::string& name, const std::filesystem::path& path);
