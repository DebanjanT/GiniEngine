...
void PostProcess::Init() {
    ...
    s_PPData->tonemapShader = Shader::Create(s_PPVertexShader, s_PPFragmentShader);
    InitQuad();
...
void PostProcess::Shutdown() {
    if (s_PPData) {
        if (s_PPData->quadVAO) glDeleteVertexArrays(1, &s_PPData->quadVAO);
        if (s_PPData->quadVBO) glDeleteBuffers(1, &s_PPData->quadVBO);
        delete s_PPData;
        s_PPData = nullptr;
    }
}
...
void PostProcess::Resolve(u32 hdrColorTexture, u32 ssaoTexture,
                          f32 exposure, f32 gamma) {
    if (!s_PPData || !s_PPData->tonemapShader) return;
