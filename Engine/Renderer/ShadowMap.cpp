...
void ShadowMap::Init() {
    ...
    s_Shadow->depthShader = Shader::Create(s_DepthVertexShader, s_DepthFragmentShader);
    ...
    GINI_INFO("Shadow mapping initialized ({}x{}, {} cascades)",
              SHADOW_MAP_SIZE, SHADOW_MAP_SIZE, NUM_CASCADES);
}

void ShadowMap::Shutdown() {
    if (s_Shadow) {
        if (s_Shadow->depthFBO) glDeleteFramebuffers(1, &s_Shadow->depthFBO);
        if (s_Shadow->depthTextureArray) glDeleteTextures(1, &s_Shadow->depthTextureArray);
        delete s_Shadow;
        s_Shadow = nullptr;
    }
}

static void ComputeCascadeSplits(f32 nearPlane, f32 farPlane, std::vector<f32>& splits) {
    f32 lambda = 0.75f;
...
static Mat4 ComputeLightSpaceMatrix(const Camera3D& camera, const DirectionalLight& light,
    ...
    f32 minZ = std::numeric_limits<f32>::max(), maxZ = std::numeric_limits<f32>::lowest();
...
void ShadowMap::BeginShadowPass(const Camera3D& camera, const DirectionalLight& light) {
    if (!s_Shadow || !s_Shadow->initialized) return;
...
void ShadowMap::BindShadowMaps(Shader* shader, u32 startTextureUnit) {
    if (!s_Shadow || !s_Shadow->initialized || !shader) return;
...
bool ShadowMap::IsInitialized() {
    return s_Shadow && s_Shadow->initialized;
}
