...
void main() {
    ...
    vec3 layer0 = texture(u_Layer0Albedo, v_TexCoord * u_Layer0Tiling).rgb;
    vec3 layer1 = texture(u_Layer1Albedo, v_TexCoord * u_Layer1Tiling).rgb;
    vec3 layer2 = texture(u_Layer2Albedo, v_TexCoord * u_Layer2Tiling).rgb;
    vec3 layer3 = texture(u_Layer3Albedo, v_TexCoord * u_Layer3Tiling).rgb;
...
static f32 InterpolatedNoise2D(f32 x, f32 y) {
    ...
    f32 v4 = SmoothNoise2D(static_cast<f32>(intX + 1), static_cast<f32>(intY + 1));
...
void Terrain::LoadHeightmap(const std::string& filepath) {
    ...
    // Resize if needed
    if (static_cast<u32>(width) != m_Width || static_cast<u32>(height) != m_Height) {
        m_Width = width;
        m_Height = height;
...
void Terrain::GenerateFromNoise(f32 frequency, f32 amplitude, i32 octaves) {
    for (u32 z = 0; z < m_Height; z++) {
        for (u32 x = 0; x < m_Width; x++) {
            ...
            m_Heightmap[z * m_Width + x] = PerlinNoise2D(nx, nz, 0.5f, octaves) * amplitude;
        }
    }
    RegenerateMesh();
...
f32 Terrain::GetHeightAtPosition(f32 worldX, f32 worldZ) const {
    ...
    if (terrainX < 0 || terrainX >= m_Width - 1 || terrainZ < 0 || terrainZ >= m_Height - 1) {
        return 0.0f;
    }
...
void Terrain::PaintHeight(f32 worldX, f32 worldZ, f32 radius, f32 strength, bool raise) {
    f32 terrainX = (worldX - m_Position.x) / m_Scale;
    f32 terrainZ = (worldZ - m_Position.z) / m_Scale;
    f32 terrainRadius = radius / m_Scale;
    
    i32 minX = std::max(0, static_cast<i32>(terrainX - terrainRadius));
    i32 maxX = std::min(static_cast<i32>(m_Width - 1), static_cast<i32>(terrainX + terrainRadius));
    i32 minZ = std::max(0, static_cast<i32>(terrainZ - terrainRadius));
    i32 maxZ = std::min(static_cast<i32>(m_Height - 1), static_cast<i32>(terrainZ + terrainRadius));
...
void Terrain::SmoothHeight(f32 worldX, f32 worldZ, f32 radius, f32 strength) {
    ...
    i32 maxX = std::min(static_cast<i32>(m_Width - 2), static_cast<i32>(terrainX + terrainRadius));
    i32 minZ = std::max(1, static_cast<i32>(terrainZ - terrainRadius));
    i32 maxZ = std::min(static_cast<i32>(m_Height - 2), static_cast<i32>(terrainZ + terrainRadius));
        ...
        for (i32 x = minX; x <= maxX; x++) {
            ...
            if (dist <= terrainRadius) {
                ...
                f32 avg = (GetHeight(x-1, z) + GetHeight(x+1, z) + 
                          GetHeight(x, z-1) + GetHeight(x, z+1)) / 4.0f;
                
                f32 current = GetHeight(x, z);
                smoothed[z * m_Width + x] = current + (avg - current) * strength * falloff;
            }
        }
    }
...
void Terrain::FlattenHeight(f32 worldX, f32 worldZ, f32 radius, f32 targetHeight) {
    f32 terrainX = (worldX - m_Position.x) / m_Scale;
    f32 terrainZ = (worldZ - m_Position.z) / m_Scale;
    f32 terrainRadius = radius / m_Scale;
    ...
    for (i32 z = minZ; z <= maxZ; z++) {
        for (i32 x = minX; x <= maxX; x++) {
            ...
            if (dist <= terrainRadius) {
                f32 falloff = 1.0f - (dist / terrainRadius);
                f32 current = GetHeight(x, z);
                m_Heightmap[z * m_Width + x] = current + (targetHeight - current) * falloff;
            }
        }
    }
...
void Terrain::PaintMaterial(f32 worldX, f32 worldZ, f32 radius, f32 strength, u32 layerIndex) {
    ...
    for (i32 z = minZ; z <= maxZ; z++) {
        for (i32 x = minX; x <= maxX; x++) {
            ...
            if (dist <= terrainRadius) {
                ...
                m_Splatmap[idx * 4 + layerIndex] = std::min(1.0f, m_Splatmap[idx * 4 + layerIndex] + addAmount);
...
void Terrain::CreateMesh() {
    ...
    for (u32 z = 0; z < m_Height; z++) {
        for (u32 x = 0; x < m_Width; x++) {
            ...
            // Calculate normal
            f32 hL = (x > 0) ? m_Heightmap[z * m_Width + (x-1)] * m_HeightScale : height;
            f32 hR = (x < m_Width-1) ? m_Heightmap[z * m_Width + (x+1)] * m_HeightScale : height;
            f32 hD = (z > 0) ? m_Heightmap[(z-1) * m_Width + x] * m_HeightScale : height;
            f32 hU = (z < m_Height-1) ? m_Heightmap[(z+1) * m_Width + x] * m_HeightScale : height;
...
void Terrain::Render(const Camera3D& camera) {
    ...
    for (u32 i = 0; i < 4; i++) {
        ...
        if (i < m_Layers.size() && m_Layers[i].albedoMap) {
...
void Terrain::SaveHeightmap(const std::string& filepath) {
    ...
    for (u32 i = 0; i < m_Width * m_Height; i++) {
        data[i] = static_cast<u8>(std::clamp(m_Heightmap[i] * 255.0f, 0.0f, 255.0f));
    }
